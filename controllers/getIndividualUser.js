...
exports.updateProcessedVideo = async (req, res) => {
  try {
    ...
    console.log(`🔄 Update video request for Reel ${req.body}`);
...
exports.uploadProfileImage = async (req, res) => {
  ...
  try {
    ...
    if (type === "profile") {
      fileName = `feed/${userId}/${timeFrame}_profileImage${extension}`;
    } else {
      fileName = `cover/${userId}/cover_image/${timeFrame}_coverImage${extension}`;
    }

    let optimizedFilename =  `optimized/${userId}/${timeFrame}_profileImage.webp` ;
...
exports.updateGymDetails = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Gym updated successfully', gym_name });
  } catch (error) {
    console.error('Error updating full name:', error);
    res.status(500).json({ message: 'Server error' });
  }
...
exports.getAllHashTag = async (req, res) => {
  const loggedInUserId = req.user.id;
  const { type, mode, tags } = req.query;

  try {
    const limit = parseInt(req.query.limit || 10);
    const offset = parseInt(req.query.offset || 0);

    // ... (keep your existing followings, friends, blocked logic)

    const visibilityConditions = [ /* your existing conditions */ ];
    const excludeCondition = excludedUserIds.length ? `AND f."userId" NOT IN (:excludedUserIds)` : '';
    ...
    const feedItems = await sequelize.query(query, {
      replacements: { /* your replacements */ loggedInUserId, limit, offset, ... },
      type: sequelize.QueryTypes.SELECT,
      nest: true,
    });

    const result = feedItems.map(feed => ({
      ...feed,
...
exports.getMyFeed = async (req, res) => {
  ...
  try {
    // Get followings, friends, and blocked users (same as before)
    const followings = await Follow.findAll({ where: { followerId: loggedInUserId }, attributes: ['followingId'] });
    const followingIds = followings.map(f => f.followingId);
    const idsArray = [loggedInUserId, ...followingIds];

    const friends = await FriendRequest.findAll({
      where: { status: 'accepted', [Op.or]: [{ fromUserId: loggedInUserId }, { toUserId: loggedInUserId }] }
    });
    const friendIds = new Set();
    friends.forEach(req => {
      if (req.fromUserId !== loggedInUserId) friendIds.add(req.fromUserId);
    ...
    const blockedBy = await Block.findAll({ where: { blockingId: loggedInUserId }, attributes: ['blockerId'] });
    const userBlocked = await Block.findAll({ where: { blockerId: loggedInUserId }, attributes: ['blockingId'] });
    const excludedUserIds = [...new Set([...blockedBy.map(b => b.blockerId), ...userBlocked.map(b => b.blockingId)])];

    const visibilityConditions = [
      `f."postType" = 'public'`,
      `(f."userId" = :loggedInUserId)`
    ];
    if (followingIds.length > 0) visibilityConditions.push(`(f."postType" = 'private' AND f."userId" IN (:ids))`);
    if (friendIdArray.length > 0) visibilityConditions.push(`(f."postType" = 'onlyme' AND f."userId" IN (:friendIds))`);
